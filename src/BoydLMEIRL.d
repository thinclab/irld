...
int main() {
    ...
    // This only works if one NE is specified
    double[Action][] selected_NE;
    
    foreach(i, p; NE_prior) {
    	if (p > 0)
    		selected_NE = NEs[i];
    }
...
//	lastWeights2 = [1, -1, .1, 0, 0];
	...
	LatentMaxEntIrlZiebartApproxMultipleAgents irl = new LatentMaxEntIrlZiebartApproxMultipleAgents(3, new ValueIteration(), observableStatesList, 50, .01, .01,interaction_delegate);
